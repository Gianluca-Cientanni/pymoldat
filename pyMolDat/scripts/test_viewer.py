...
import sys
from functions import extract_info, atomic_pairs
...
class E(tkinter.Tk):
    ...
    def main(self):
        ...
        # define axes for matplotlib widget
        ax = Axes3D(self.fig)
        ...
        # select file button
        self.btn = tkinter.Button(self, text='Select File', command=self.alt)
        self.btn.pack(ipadx=250)

        # display text file
        # self.text_file = tkinter.Text(self, state='normal', height='15', width='60')
        # self.text_file.pack(ipadx=250)
        #self.text_file.insert(self, open(file_dir).read)

    # directory navigator button (a la zortero)

    # mini window displaying key information extracted from files (see sheet)
    def alt (self):
        print('benzene')
    def dest(self):
        self.destroy()
